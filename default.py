...
def indexLiveTV():
    ...
    for channel in res['data']['channelList']['channels']['edges']:
        if channel['node']['currentEvent']['items']:
            ...
            if not(adult) or (adult and adult_setting):
                ...
                url = build_url(query)
                pl = pl + '#EXTINF:-1 tvg-chno="%s" tvg-logo="%s" tvg-id="%s",%s\n%s\n' % (str(300+countv),channel['node']['logo']['url'], channel['node']['id'], channel['node']['title'],url)
    ...
    f_m3u =  open(os.path.join('/storage/emulated/0/Android/data/org.xbmc.kodi/files/.kodi/userdata/addon_data/plugin.program.createbulsatcomplaylist/dat/', '', 'a1.m3u'), 'wb+')
